...
function showOptions() {
    ...
    $("#setting_aa_clicklisteners").hide();
    ...
    if ($("#radio_codetype_mv").is(":checked")) {
        ...
        $("#setting_aa_clicklisteners").show();
    }
}
...
function generateGACode() {
    var result = "\n";
    var lines = $("#values_input").val().split('\n');
    for (var i = 0; i < lines.length; i++) {
        var items = lines[i].split(':');
        var last = items[items.length-1];

        var varName = last.replace(/-/g, '_').toUpperCase();
        result+="public static final String "+varName+" = \""+last+"\";\n"
    }
...
//	});
    ...
    $("#values_input").bind("keyup paste", function (e) {
        generateGACode();
    });
    ...
    $("#chk_support, #chk_includepackage, #chk_dontcamelcase, #chk_mv_parentviewparam, #chk_mv_clicklisteners, #chk_aa_clicklisteners").change(function () {
        generateJavaFromTree();
    });
    ...
    $("#radio_codetype_mv, #radio_codetype_vh, #radio_codetype_aa, #radio_codetype_ca, #radio_codetype_android_annotations").change(function () {
        showOptions();
        generateJavaFromTree();
    });
